...
void addPrimfactor(long int num,primFactor* p){
...
primFactor* getPrimfactors(long int num){
    primFactor* p=malloc(sizeof(primFactor));
    p->next=p;
    p->prev=p;
    p->val=0;
    long int maxNum=((long int)sqrt(num))+1;

    for (long int i = 2; i <= maxNum && num!=1; i++){
       if (num%i==0){
           if (!p->val){
                p->val=i;
                p->pow=1;
...
long int getKGV(long int numA,long int numB){
    ...
    do {
        if(p1->val==p2->val){
            ...
            kgv*=((long int)pow(p1->val,biggest));
            p1=p1->next;
            p2=p2->next;
            p1t=0;p2t=0;
        } else if((p1->val<p2->val || (p2==p2i && !p2t)) && (p1!=p1i || p1t)){
            kgv*=((long int)pow(p1->val,p1->pow));
            p1=p1->next;
            p1t=0;
        } else if((p1->val>p2->val || (p1==p1i && !p1t)) && (p2!=p2i || p2t)){
            kgv*=((long int)pow(p2->val,p2->pow));
            p2=p2->next;
            p2t=0;
        }
